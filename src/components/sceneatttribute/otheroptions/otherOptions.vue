...
<script>
import mapOptions from "../../viewer/mapOptions.js"
...
export default {
  ...
  mounted() {
    ...
    if(mapOptions.getUrlKey("type") == 1) {
      this.init()
      this.funcRoll(true)
    }
  },
  methods: {
    ...
    imageryRoller(val){
      ...
      let horizontalSlider = document.getElementById("horizontalSlider");
      if (val) {
        blackMarble = viewer.imageryLayers.addImageryProvider(new Cesium.createOpenStreetMapImageryProvider({

<template>
  <div class="myWidgetContainer" v-show="measureComb">
    <div class="contentHeader">
      <span>{{ Resource.measure }}</span>
      <span class="closeBtn" @click="toggleVisibility">&times;</span>
    </div>
    <div class="contentBody" style="padding: 0">
      <div class="sm-function-module-content">
        <div v-if="isXianzhi" class="sm-measure-btn-list">
          <button @click="areaClk" class="sm-btn sm-measure-btn" id="area" :title="'绘制分析'" type="button">
        ...
        <div v-else class="sm-measure-btn-list">
          <button @click="distanceClk" class="sm-btn sm-measure-btn" id="distance" :title="Resource.SpatialDistance" type="button">
            <span class="iconfont iconVue-measure-distance" style="font-size:22px;"></span>
          </button>
          <button @click="GroundDistanceClk" class="sm-btn sm-measure-btn" id="GroundDistance" :title="Resource.GroundDistance" type="button">
          ...
          <button @click="GroundAreaClk" class="sm-btn sm-measure-btn" id="GroundArea" :title="Resource.FloorArea" type="button">
          ...
          <button @click="ProjectArea" class="sm-btn sm-measure-btn" id="ProjectArea" :title="Resource.shadowArea" type="button">
...
<script>
import mapOptions from '../../viewer/mapOptions'
...
export default {
  ...
  methods: {
      ...
      screenSpaceEventHandler.setInputAction(function (movement) {
        if (
          ...
          that.polygonInstance.geometry._polygonHierarchy.positions =
            that.polygonPositions
          var geometries = []
          for (var i = 0; i < that.polygonPositions.length; i++) {
            var position = that.polygonPositions[i]
    ...
    query(queryPoints) {
      let that = this
      that.inputGeom = queryPoints
      var queryObj = {
        getFeatureMode: 'SPATIAL',
        spatialQueryMode: 'INTERSECT',
        datasetNames: ['chuz:dantihua', 'chuz:DLTB'],
        geometry: {
          points: queryPoints,
          type: 'REGION'
        }
      ...
      window.axios
        ...
        .then(function (result) {
          ...
          if (resultObj.featureCount > 0) {
            let spatiaRes = { jbnt: [], zjd: [] }
            let inputGeom = that.inputGeom.map((item, index, arr) => {
              return [item.x, item.y]
            })
            if((inputGeom[0][0] != inputGeom[inputGeom.length -1][0]) || (inputGeom[0][1] != inputGeom[inputGeom.length -1][1])) inputGeom.push([...inputGeom[0]])
            resultObj.features.forEach((feature) => {
              let featGeom = feature.geometry.points.map((item, index, arr) => {
                return [item.x, item.y]
              })
              ...
              let _curArea = area(intersection)
              if (feature.fieldNames.indexOf('DLBM') >= 0) {
                let _res = { type: '', area: 0 }
                _res.type = feature.fieldValues[feature.fieldNames.indexOf('DLBM')].substring(0, 2) || '01'
                _res.area = _curArea
                spatiaRes.jbnt.push(_res)
              } else if (feature.fieldNames.indexOf('ZJDDM') >= 0) {
                let _res = { zjddm: '', area: 0, lyfs: '01' }
                _res.zjddm = feature.fieldValues[feature.fieldNames.indexOf('ZJDDM')],
                _res.area = _curArea
                _res.lyfs = feature.fieldValues[feature.fieldNames.indexOf('ZJDLYFS')] || '01',
                spatiaRes.zjd.push(_res)
              }
            })
            mapOptions.SendUIMessage('queryRes', spatiaRes)
            that.drawFeats(resultObj.features)
          }
        })
    ...
    drawFeats(features) {
      for (let index = 0; index < features.length; index++) {
        ...
        var lonLatArr = mapOptions.getLonLatArray(feature.geometry.points)
        viewer.entities.add({
          flag: 'query',
          polygon: {
    ...
    clearLine() {
      ...
      for (var i = 0; i < this.layers._layers._array.length; i++) {
        let hypsometricSetting =
          this.layers._layers._array[i].hypsometricSetting.hypsometricSetting
        if (Cesium.defined(hypsometricSetting)) {
          this.layers._layers._array[
            i
          ].hypsometricSetting.hypsometricSetting.MaxVisibleValue = -100
        }
      }
    },
    deactiveAll() {

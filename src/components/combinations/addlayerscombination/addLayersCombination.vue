<template>
  <div class="myWidgetContainer" v-show="addLayerShow" >
    <div class="contentHeader">
      <span>{{ Resource.addLayer }}</span>
      <span  class="closeBtn"  @click="toggleVisibility">&times;</span>
    </div>
    <div class="contentBody" id="terrainAnalysis"   data-attr="drag" style="padding: 0">
      <div class="sm-panel-header">
        <span :class="{titleColor:webServiceShow}" class="title-txt" @click="choose(0)">{{Resource.publicService}}</span>
        <span :class="{titleColor:customServiceShow}" class="title-txt" @click="choose(1)">{{Resource.customService}}</span>
        <span :class="{titleColor:localFileShow}" class="title-txt" @click="choose(2)">{{Resource.localKML}}</span>
      </div>
      <!-- 调用子组件 -->

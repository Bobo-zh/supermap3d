...
export default {
  ...
  methods: {
    init() {
      ...
      let _code = mapOptions.getUrlKey('code')
      ...
      this.addLayer()
      if(mapOptions.getUrlKey("type") == 1) this.mapConfig.position = [-2533220.345797352, 4738043.009387192, 3446274.7552214935]
      viewer.camera.flyTo({
    ...
    addDantihua() {
      ...
      promise.then((layer) => {
        ...
        mapOptions.SendUIMessage('3dMapLoaded', '三维加载完毕')
    ...
    async getAttrData(zjddm, suyqrdm, zdInfos) {
      ...
      mapOptions.SendUIMessage('mapInfos', {
        type: 1,
        data: zdInfos,
        mode: 3
      })

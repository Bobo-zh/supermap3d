...
let LatAndLons = [],Cartesians = [],EditPositions = null; //所有点的经纬度，笛卡尔坐标及编辑时保存之前的位置
export default {
  name: "Sm3dProfile",
  props: {},
  data() {
  ...
  methods: {
    ...
    initMyChart(){
      if(window.echarts){
         if(!myChart){
            myChart = window.echarts.init(document.getElementById("profile3D_echarts")); //初始化echarts
    ...
    analysis() {
      ...
      common.handlerDrawing("Polyline").then(
        res =>{
          ...
          //起止点相关信息
          var scartographic = Cesium.Cartographic.fromCartesian(res.result.object._positions[0]);
          ...
          var ecartographic = Cesium.Cartographic.fromCartesian(res.result.object._positions[1]);
          ...
          that.DrawPolylineUpdate(res.positions,res.result.object._positions);
          ...
          that.updataProfile3D('',res.result.object);   //更新剖面
          common.Edit(EditPositions,that.updataProfile3D,"Polyline"); //编辑功能
        },
        (err)=>{
          console.log(err);
        }
    ...
    DrawPolylineUpdate(position,resultPosition){
        ...
        this.entityUpdate(resultPosition);
    },
    updataProfile3D(position,line){     //更新剖面分析二维图
      let that = this;
      ...
      for(let i=1,j = line._positions.length;i<j;i++){
        ...
        for(let i =1,j = count;i<j;i++){
          let cart = Cesium.Cartesian3.lerp(startPoint, endPoint, i/count, new Cesium.Cartesian3());
          positions.push(cart);
        }
      }
